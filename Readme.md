...
# python库依赖（建议按装在本项目手动自建的.venv环境下）
ros2相关库
numpy
transform3d
sros_sdk_py(底盘控制依赖，需要手动安装)

## sros_sdk_py手动安装到.venv

# 确保系统有 venv 组件（如果提示已安装则跳过）
sudo apt install python3-venv -y
# 创建虚拟环境（这时会自动安装好 pip）
python3 -m venv .venv
# 安装对应库
.venv/bin/python -m pip install ./src/amr_control/sros_sdk_py-1.4.1-py3-none-any.whl
